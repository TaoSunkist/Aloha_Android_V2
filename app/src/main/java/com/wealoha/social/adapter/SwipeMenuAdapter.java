...
public class SwipeMenuAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		picasso.load(ImageUtil.getImageUrl(user.avatarImage.id, GlobalConstants.ImageSize.AVATAR_ROUND_SMALL, CropMode.ScaleCenterCrop)).placeholder(R.drawable.default_photo).into(holder.userPhoto);
		holder.userName.setText(user.name);
		if (showMatch && user.match) {
			holder.matchOrNot.setVisibility(View.VISIBLE);
		} else {
			holder.matchOrNot.setVisibility(View.GONE);
		}
		holder.userPhotoCount.setText(context.getString(R.string.aloha_time_photo_count, user.postCount));
		holder.userInfos.setText(user.age + "·" + user.height + "·" + user.weight + "·" + StringUtil.getUserZodiac(user.zodiac, context));
		return convertView;
	}
