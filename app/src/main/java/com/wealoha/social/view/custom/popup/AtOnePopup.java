...
public class AtOnePopup implements OnTouchListener, OnClickListener, OnGestureListener {
	...
	public AtOnePopup(Context context, ViewGroup container, UserTags userTags) {
	...
	public void initAtPopup(boolean canMove, AtOnePopupCallback callback) {
		...
		float initX = mUserTags.tagAnchorX * mContainerWidth;
		float initY = mUserTags.tagAnchorY * mContainerHeight;
	...
	private void openProFile() {
		...
		bundle.putParcelable(User.TAG, mUserTags.tagUser);
		if (contextUtil.getForegroundAct() != null) {
			((BaseFragAct) contextUtil.getForegroundAct()).startFragmentHasAnim(ProfileTestFragment.class, bundle, true, R.anim.left_in, R.anim.stop);
		}
	}
