...
public class FeedItemHolder implements OnClickListener, ListItemCallback {
	...
	private void showLikeBar() {
		...
		if (getLikeCount() == 0) {
			...
			// 只有别人赞了
			// xx 個人說贊
			mPraiseList.setText(context.getString(R.string.feed_like_n_people_like, getLikeCount()));
			mPraiseList.setVisibility(View.VISIBLE);
			mPraiseLine.setVisibility(View.VISIBLE);
		} else if (getLikeCount() == 1 && mFeed.liked) {
			// 只有自己贊了
			...
			// 你和別人贊了
			mPraiseList.setText(context.getString(R.string.feed_like_you_and_n_people_like, getLikeCount() - 1));
			mPraiseList.setVisibility(View.VISIBLE);
			mPraiseLine.setVisibility(View.VISIBLE);
		}
	}
