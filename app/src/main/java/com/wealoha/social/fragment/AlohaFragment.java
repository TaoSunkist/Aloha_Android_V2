...
public class AlohaFragment extends BaseFragment implements LoaderCallbacks<Result<ResultData>>, ListItemCallback {
	...
	private void openPopReport() {
		...
		// PostType.USER, user.id, user.name, user);
		new ListItemDialog(getActivity(), (ViewGroup) view).showListItemPopup(this, user.name, ListItemType.REPORT_USER);
	}
	 ...
	 */
	void reportUser() {
		mUserService.reportUser(user.id, new Callback<Result<ResultData>>() {
	...
	void nopeOrAloha(Action action) {
		...
		case Aloha:
			...
			if (toUser != null && toUser.id.equals(tempUser.id)) {
				mIsNope = false;
				returnNextUser(null);
			} else if (toUser == null) {
				mIsNope = false;
	...
	void displayUser() {
		mUsername.setText("" + user.name);
		mWord.setText(user.summary);

		if (user.region == null || user.region.size() == 0) {
			mFind.setText(R.string.location_invisible);
		} else {
			String region = "";
			for (String r : user.region) {
				region += r + ", ";
			}
		...
		if (user.postCount > 0) {
			...
			mDes.setText(mContext.getString(R.string.aloha_time_photo_count, user.postCount));
		} else {
			mDes.setVisibility(View.INVISIBLE);
		}
		mTag.setVisibility(View.VISIBLE);
		mTag.setText(mBaseFragAct.getUserTag(user.id) == null ? "" : mContext.getString(R.string.aloha_user_profile_tag, mBaseFragAct.getUserTag(user.id)));
		String brith = user.age + " · " + user.height + " · " + user.weight;
		String userTag = ContextUtil.getUserTag(user.selfTag);
		if (userTag != null) {
			brith += " · " + userTag;
		}

		brith += " · " + StringUtil.getUserZodiac(user.zodiac, mContext);
		mBrith.setText(brith);
		// 寻找
		String purposes = ContextUtil.formatPurposes(user.selfPurposes);
		...
		String imageUrl = ImageUtil.getImageUrl(user.avatarImage.id, imageWidth, CropMode.ScaleCenterCrop);
		...
		if (AppApplication.mUserList.size() > 1 && AppApplication.mUserList.get(1) != null) {
			...
			String nextImageUrl = ImageUtil.getImageUrl(nextUser.avatarImage.id, imageWidth, CropMode.ScaleCenterCrop);
