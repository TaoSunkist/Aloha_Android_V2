...
public class SwipeMenuListFragment extends BaseFragment implements ListItemCallback, OnItemClickListener,
	...
	private void initData() {
		...
		if (mListType == LISTTYPE_LIKE) {
			mTitle.setText(getResources().getString(R.string.profile_aloha_title, contextUtil.getCurrentUser().alohaCount));
			loadLiksList();
			...
			// 处理标题栏显示的人气数量
			mTitle.setText(getResources().getString(R.string.profile_aloha_get_title, contextUtil.getCurrentUser().alohaGetCount));
	...
	private void deleteFromLikeList(final int position) {
		...
		mUserService.dislikeUser(mUsers.get(position).id, new Callback<Result<ResultData>>() {
			...
			public void success(Result<ResultData> result, Response arg1) {
				...
				if (mCurrentUser.alohaCount > 0) {
					...
					mTitle.setText(getResources().getString(R.string.profile_aloha_title, mCurrentUser.alohaCount));
		...
		mProfileService.view(userid, new Callback<Result<ProfileData>>() {
			...
			public void success(Result<ProfileData> result, Response arg1) {
				...
				for (User user : mUsers) {
					if (user.id.equals(userid)) {
						user.match = result.data.user.match;
						mSwipeAdapter.notifyDataSetChangedByList(mUsers);
						break;
					}
				}
