...
public class ConfigDetailsAct extends BaseFragAct implements LoaderCallbacks<Result<ProfileData>> {
	...
	public void onClick(View v) {
		...
		case R.id.config_details_introduction:
			...
			intent.putExtra("content", contextUtil.getCurrentUser() != null ? contextUtil.getCurrentUser().summary : "");
	...
	private boolean checkInfoChange() {
		...
		if (mUser.name != null && !mUser.name.equals(mTempUser.name)) {
			return false;
		}

		if (mUser.weight != null && !mUser.weight.equals(mTempUser.weight)) {
			return false;
		}
		if (mUser.height != null && !mUser.height.equals(mTempUser.height)) {
			return false;
		}
		...
		// return false;
		// }
		if (mUser.birthday != null && !mUser.birthday.equals(mTempUser.birthday)) {
			return false;
		}
		if (mUser.selfTag != null && !mUser.selfTag.equals(mTempUser.selfTag)) {
		...
		if (mUser.selfPurposes != null && mTempUser.selfPurposes == null) {
			...
			// "-----mUser.selfPurposes---------");
			return false;
		} else if (mUser.selfPurposes == null && mTempUser.selfPurposes != null) {
			// ToastUtil.longToast(mContext,
			// "-----mTempUser.selfPurposes---------");
			return false;
		}

		if (!StringUtil.join("-", mUser.selfPurposes).equals(StringUtil.join("-", mTempUser.selfPurposes))) {
			return false;
		}
		...
		// }
		// }
		if (mUser.avatarImageId != null && !TextUtils.isEmpty(mTempUser.avatarImageId)) {
			if (!mUser.avatarImageId.equals(mTempUser.avatarImageId)) {
				return false;
			}
		}
	...
	public void save() {
		...
		// 为了和ios兼容，使用『-』拼起来
		String selfPurpose = StringUtil.join("-", user.selfPurposes);
		userService.reqAlertUserInfo(user.name, user.birthday, user.height, user.weight, user.selfTag, user.regionCode, selfPurpose, user.avatarImageId, user.summary, new Callback<Result<AuthData>>() {
		...
		case LOADER_GET_PROFILE:
			...
			return new AsyncLoader<Result<ProfileData>>(this) {
				...
				public Result<ProfileData> loadInBackground() {
					try {
						return profileService.view(contextUtil.getCurrentUser().id);
					} catch (Exception e) {
						return null;
					}
				}
	...
	public void initData() {
		...
		picasso.load(ImageUtil.getImageUrl(mUser.avatarImage.id, ImageSize.CHAT_THUMB, CropMode.ScaleCenterCrop)).placeholder(R.drawable.default_photo).into(mUserPhoto);
		mUserName.setText(TextUtils.isEmpty(mUser.name) ? "" : mUser.name);

		// 年龄
		mAgeTv.setText(TextUtils.isEmpty(mUser.age) ? "" : mUser.age);
		mAgeTv.setTag(TextUtils.isEmpty(mUser.birthday) ? "" : mUser.birthday);
		// 身高
		mHeightTv.setText(TextUtils.isEmpty(mUser.height) ? "" : mUser.height + "cm");
		mHeightTv.setTag(TextUtils.isEmpty(mUser.height) ? "" : mUser.height + "");
		// 体重
		mWeightTv.setText(TextUtils.isEmpty(mUser.weight) ? "" : mUser.weight + "kg");
		mWeightTv.setTag(TextUtils.isEmpty(mUser.weight) ? "" : mUser.weight);
		// introduction
		mIntroductionTv.setText(TextUtils.isEmpty(mUser.summary) ? "" : mUser.summary);
		...
		if (mUser.selfPurposes != null && mUser.selfPurposes.size() > 0) {
			CheckBox[] cbs = new CheckBox[] { mGoal01, mGoal02, mGoal03 };
			for (String purposes : mUser.selfPurposes) {
				String[] ps = StringUtil.split(purposes, "-");
				if (ps != null && ps.length > 0) {
					ONE_PURPOSE: for (String p : ps) {
						XL.d(TAG, "purpose: " + p);
						for (CheckBox cb : cbs) {
		...
		// selftag
		if (!TextUtils.isEmpty(mUser.selfTag)) {
			for (int i = 0; i < mStatureRg.getChildCount(); i++) {
				RadioButton radio = ((RadioButton) mStatureRg.getChildAt(i));
				if (radio.getTag().equals(mUser.selfTag)) {
