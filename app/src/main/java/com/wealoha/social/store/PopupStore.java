...
public class PopupStore {
		...
		OnClickListener sharePopOnClick = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case R.id.popu_outside_ll:
					break;
				case 0:
					title = baseFragAct.getString(toUser == null ? R.string.share_to_wx_title : R.string.share_to_other_wx_title, toUser == null ? meUser.name : toUser.name);
					...
					ShareStore.shareToWXHasUrl(baseFragAct, url, title, body, toUser == null ? ImageUtil.getImageUrl(meUser.avatarImage.id, ImageSize.CHAT_THUMB, CropMode.ScaleCenterCrop) : ImageUtil.getImageUrl(toUser.avatarImage.id, ImageSize.AVATAR, CropMode.ScaleCenterCrop));
					break;
				case 1:
					title = baseFragAct.getString(toUser == null ? R.string.share_to_wx_title : R.string.share_to_other_wx_title, toUser == null ? meUser.name : toUser.name);
					body = StringUtil.shareDescription(toUser == null ? meUser : toUser, baseFragAct, mRegionNodeUtil);
					...
					ShareStore.shareToFriendHasUrl(baseFragAct, url, title, body, toUser == null ? ImageUtil.getImageUrl(meUser.avatarImage.id, ImageSize.CHAT_THUMB, CropMode.ScaleCenterCrop) : ImageUtil.getImageUrl(toUser.avatarImage.id, ImageSize.AVATAR, CropMode.ScaleCenterCrop));
					break;
				case 2:
					title = baseFragAct.getString(R.string.share_to_other_wx_title, toUser == null ? meUser.name : toUser.name);
				...
				case 4:
					...
					if (toUser == null) {
						user2 = meUser;
						username2 = meUser.name;
						imgid2 = meUser.avatarImage.id;
						titleid2 = R.string.share_to_wx_title;
						urlid2 = StringUtil.ME_PROFILE_TO_SHARE_WXF;
					} else {
						user2 = toUser;
						username2 = toUser.name;
						imgid2 = toUser.avatarImage.id;
		...
		OnClickListener sharePopOnClick = new OnClickListener() {
			...
			public void onClick(View v) {
				...
				case 0:
					...
					// toUser == null ?
					// ImageUtil.getImageUrl(meUser.avatarImage.id,
					// ImageSize.CHAT_THUMB,
					// CropMode.ScaleCenterCrop) :
					// ImageUtil.getImageUrl(toUser.avatarImage.id,
				...
				case 3:
					...
					url = StringUtil.sharePostWebUrl(post.getUser().getId(), toUser.id, post, StringUtil.OTHER_PROFILE_TO_SHARE_WXF);
	...
	private void black(final User user) {
		mUserService.blackUser(user.id, new Callback<Result<ResultData>>() {
	...
	private void removeFromBlack(final User user) {
		mUserService.unblock(user.id, new Callback<Result<ResultData>>() {
	...
	private void report(User user) {
		mUserService.reportUser(user.id, null, new Callback<Result<ResultData>>() {
				...
				case 0:
					if (mPost.getType() == FeedType.ImagePost) {
						if (mPost.isMine()) {// 分享自己的
							...
							ShareStore.shareImgToWXHasUrl(baseFragAct, StringUtil.sharePostWebUrl(contextUtil.getCurrentUser().id,//
						...
						if (mPost.isMine()) {// 分享自己的
							...
							ShareStore.shareVideoToWXHasUrl(baseFragAct, StringUtil.sharePostWebUrl(contextUtil.getCurrentUser().id,//
																									mPost.getUser().getId(), mPost, StringUtil.OTHER_PROFILE_TO_SHARE_WX), "Aloha", "", imgUrl);
						}
					}
					feedShare(mPost);
				...
				case 1:
					...
					if (mPost.getType() == FeedType.ImagePost) {
						if (mPost.isMine()) {// 分享自己的
							...
							urlwx = StringUtil.sharePostWebUrl(contextUtil.getCurrentUser().id, //
