...
public class StringUtil {
	...
	public static String processString(String str, int length, Context context) {
		...
		WindowManager systemService = (WindowManager) context.getSystemService("window");
	...
	/***
	 * {@link #shareUserWebUrl(User, User, int)} or
	 * {@link #sharePostWebUrl(User, User, Post, int)}
	 * 
	 * @param user
	 * @param toUser
	 * @param tag
	...
	public static String shareWebPagerUrl(User user, User toUser, int tag) {
		...
		case ME_PROFILE_TO_SHARE_WX:
			...
			stringBuilder.append("?shareby=" + user.id);
			stringBuilder.append("&from=singlemessage&isappinstalled=1");
			return stringBuilder.toString();
		case ME_PROFILE_TO_SHARE_WXF:
			...
			return stringBuilder.toString();
		case OTHER_PROFILE_TO_SHARE_WX:
			stringBuilder.append(toUser.id);
			stringBuilder.append("?shareby=" + user.id);
			stringBuilder.append("&from=singlemessage&isappinstalled=1");
			return stringBuilder.toString();
		case OTHER_PROFILE_TO_SHARE_WXF:
	...
	public static String shareDescription(User user, Context context, RegionNodeUtil mRegionNodeUtil) {
		if (user != null) {
			...
			List<String> regionNames = mRegionNodeUtil.getRegionNames(user.regionCode, 10);
			if (regionNames == null) {
				return "";
			}
			...
			sBuilder.append("\n");
			sBuilder.append(user.age);
			sBuilder.append("·");
			sBuilder.append(user.height);
			sBuilder.append("·");
			sBuilder.append(user.weight);
			if (TextUtils.isEmpty(user.zodiac)) {
				return sBuilder.toString();
			} else {
				sBuilder.append("·");
				sBuilder.append(getUserZodiac(user.zodiac, context));
				return sBuilder.toString();
			}
		} else {
			return "";
