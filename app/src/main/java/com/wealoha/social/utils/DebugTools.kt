...
import com.wealoha.social.BuildConfig
import java.util.*

fun printf(vararg args: Any) {
    if (BuildConfig.DEBUG) {
        Log.d("NextVideo", args.joinToString(" "))
    }
}
...
class Debug {
    companion object {
        ...
        object ColourUtil {
            var colourArray = ArrayList<Int>().apply {
                add(color.holo_blue_bright)
                add(color.holo_blue_dark)
                add(color.holo_blue_light)
                add(color.holo_green_dark)
                add(color.holo_green_light)
                add(color.holo_orange_dark)
                add(color.holo_orange_light)
                add(color.holo_purple)
                add(color.holo_red_dark)
                add(color.holo_red_light)
            }
        }
    }
}
